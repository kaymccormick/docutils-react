...
//    console.log(`${me.count}: ${tags.join(' > ')}`);
    ...
    while ((node = nodes.pop()) && node.name) {
        ...
        if (Component) {
            const siblings = context.siblings[i].map(f => f());
            if (lastComponent) {
...
class Viewer extends Component {
    ...
    getDocumentUrl() {
        return (docName => `${document.location.origin + (this.props.src || '')}/${docName}.xml`);
    }
    ...
    render() {
        let linksComplete = false; let
docComplete = false;
        let linksFail = false; let
docFail = false;
        const onComplete = this.props.onComplete;
        const me = this;
